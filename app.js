...
function updateLikeUI(id, count, isLiked) {
  ...
  const active =
    isLiked !== undefined ? isLiked : state.likedIds.has(id);
...
window.renderFeatured = function (problemId = null) {
    ...
    <div class="relative overflow-hidden rounded-3xl bg-stone-900 text-stone-50 shadow-2xl flex flex-col md:flex-row min-h-[400px]">
      ...
      <div class="w-full md:w-7/12 p-8 md:p-12 flex flex-col justify-center relative z-10">
        ...
        <div class="flex flex-wrap gap-4 mt-auto">
          <button
            onclick="window.toggle(${prob.id})"
            class="px-6 py-3 rounded-xl font-bold text-sm transition-all shadow-md transform active:scale-95 flex items-center gap-2 ${
              isSolved
          ...
          <button
            onclick="window.renderFeatured()"
            class="px-6 py-3 rounded-xl font-medium text-stone-400 hover:text-white hover:bg-stone-800 transition-colors border border-stone-700 flex items-center gap-2"
          >
        ...
        <div class="mt-8 border-t border-stone-700/40 pt-4">
          ...
          <div
            id="featured-comments-${prob.id}"
            class="comments-list space-y-2 max-h-40 overflow-y-auto pr-1 text-sm text-stone-100/90"
          ></div>
  ...
  const featuredBtn = document.getElementById(
    `featured-comment-btn-${prob.id}`
  );
  if (featuredBtn && featuredInput) {
    featuredBtn.addEventListener("click", () =>
      addComment(prob.id, featuredInput)
    );
  }
...
function renderFeed() {
  ...
  if (state.currentFilter === "liked") {
    ...
    document.getElementById("no-results-text").innerText =
      "هنوز هیچ معمایی را لایک نکرده‌اید.";
  ...
  sorted.forEach((p) => {
    ...
    card.innerHTML = `
      <div class="visual-col bg-stone-50 h-48 flex items-center justify-center p-6 relative overflow-hidden border-b border-stone-100 text-stone-600">
        ...
        ${
          isSolved
            ? '<div class="absolute inset-0 bg-emerald-500/10 flex items-center justify-center backdrop-blur-[1px]"><span class="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold shadow-sm">حل شده</span></div>'
            : ""
        }
      </div>
      ...
      <div class="p-6 flex flex-col flex-grow">
        <div class="flex justify-between items-start mb-3">
          <span class="text-[10px] font-bold text-stone-500 uppercase tracking-wider bg-stone-100 px-2 py-1 rounded-md">
            ${displayCat}
          </span>
        </div>
        ...
        <div class="mt-auto flex items-center justify-between pt-4 border-t border-stone-50">
          <div class="flex items-center gap-2">
            <button
              onclick="window.toggle(${p.id})"
              class="text-xs font-semibold px-4 py-2 rounded-lg transition-all ${
                isSolved
                  ? "text-emerald-600 bg-emerald-50 hover:bg-emerald-100"
                  : "text-stone-600 bg-stone-100 hover:bg-stone-200 hover:text-stone-900"
              }"
            >
            ...
            <button
              onclick="window.showFull(${p.id})"
              class="text-[11px] font-semibold px-3 py-2 rounded-lg border border-stone-200 text-stone-600 hover:bg-stone-100 hover:text-stone-900 transition-all"
            >
              مشاهده کامل
            </button>
          </div>
          ...
          <button
            id="like-btn-${p.id}"
            onclick="window.handleLike(${p.id})"
            class="group/like flex items-center gap-1.5 text-stone-400 hover:text-rose-500 transition-colors p-1.5 rounded-full hover:bg-rose-50"
            title="${
              isLiked ? "حذف از مورد علاقه" : "افزودن به مورد علاقه"
            }"
          >
            <span
              id="like-count-${p.id}"
              class="text-xs font-mono font-bold pt-0.5 group-hover/like:text-rose-500"
            >
              0
            </span>
            <svg
        ...
        <div class="mt-4 border-t border-stone-100 pt-3">
          <div
            id="comments-${p.id}"
            class="comments-list space-y-2 max-h-32 overflow-y-auto pr-1 text-xs text-stone-700"
          ></div>
          <div class="flex items-center gap-2 mt-2">
            <input
              data-role="card-comment-input"
              type="text"
    ...
    if (cardBtn && cardInput) {
      cardBtn.addEventListener("click", () => addComment(p.id, cardInput));
    }
